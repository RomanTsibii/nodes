...
kill $(ps aux | grep bot_gaia.sh | grep -v grep | awk '{print $2}')   
/root/gaianet/bin/gaianet stop 
systemctl stop multiple.service
systemctl disable multiple.service
systemctl stop cysic.service
systemctl disable cysic.service
docker rm -f titan
