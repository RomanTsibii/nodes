#!/bin/bash
# bash <(curl -s https://raw.githubusercontent.com/RomanTsibii/nodes/main/ritual/install.sh) token
# 
# логи
# docker compose -f $HOME/infernet-container-starter/deploy/docker-compose.yaml logs -f --tail=100
...
if [ -n "$2" ]; then
  ...
  read -p "Введите ваш приватный ключ начинающийся с 0x" PRIVATE_KEY
fi
...
wget -4 https://raw.githubusercontent.com/DOUBLE-TOP/guides/main/ritual/docker-compose.yaml
docker compose up -d

docker rm -fv infernet-anvil  &>/dev/null
sleep 5
source .profile && bash <(curl -s https://raw.githubusercontent.com/AndriiKok/grafana-node-checker/refs/heads/main/Source/Ritual/update.sh)
source .profile && bash <(curl -s https://raw.githubusercontent.com/AndriiKok/grafana-node-checker/refs/heads/main/Source/Ritual/ritual_checker.sh)
