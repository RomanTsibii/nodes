...
# bash <(curl -s https://raw.githubusercontent.com/RomanTsibii/nodes/main/massa/update_14.sh)
...
function massa_backup {
	cd $HOME
	if [ ! -d $HOME/massa_backup_14/ ]; then
		mkdir -p $HOME/massa_backup_14
		cp $HOME/massa/massa-node/config/node_privkey.key $HOME/massa_backup_14/
		cp $HOME/massa/massa-client/wallet.dat $HOME/massa_backup_14/
	fi
	if [ ! -e $HOME/massa_backup_14.tar.gz ]; then
...
function keys_from_backup {
	cp $HOME/massa_backup_14/wallet.dat $HOME/massa/massa-client/wallet.dat
	cp $HOME/massa_backup_14/node_privkey.key $HOME/massa/massa-node/config/node_privkey.key
}
